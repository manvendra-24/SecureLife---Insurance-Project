...
import { getAllEmployees, activateEmployee, deactivateEmployee, registerEmployee } from '../../../services/AdminService';
...
const ViewEmployee = () => {
  ...
  const activeEmployee = async (id) => { 
    try {
      ...
      showToast('Error in activating employee','error'); 
    }
  };
  ...
  const deleteEmployee = async (id) => { 
    try {
      ...
      showToast('Error in deactivating employee','error');
    }
  };
  ...
  return (
    <Container fluid className="bg-light text-dark d-flex flex-column min-vh-100 px-0">
      <Header/>
      <AddEmployee show={showModal} handleClose={handleCloseModal} handleSave={handleSaveEmployee} /> 
      <Container fluid className="py-5" style={{ backgroundColor: 'rgba(230, 242, 255, 0.5)' }}>
        <Row className="m-5">
          ...
          <Col md={4} style={{ textAlign: 'right' }}>
            <NewButton text={"Add New Employee"} handleButton={handleShowModal} /> 
          </Col>
        </Row>
        <Row className="m-5">
        ...
        <div>
          <Row className="m-5">                
            <TableData data={data} fetchData={fetchData} status={"Employee"} activateRow={activeEmployee} deactivateRow={deleteEmployee} />
          </Row>
          <Row className="m-5">
            <Col md={6}>
