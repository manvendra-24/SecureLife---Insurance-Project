...
import { getAllTypes, activateType, deactivateType } from '../../../services/AdminService';
...
const ViewInsuranceType = () => {
  ...
  const activeType = async (id) => {
    try {
      ...
      showToast('Type Activated successfully', 'success');
      setTimeout(()=>{
        fetchData();
      },1000);
    } catch (error) {
      showToast('Error in activating Insurance Type', 'error');
    }
  };
  ...
  const deleteType = async (id) => {
    try {
      ...
      showToast('Error in deactivating Insurance Type', 'error');
    }
  };

  const getSchemesByType = (id) =>{
    navigate(`/SecureLife.com/admin/types/${id}/schemes`)
  }
  ...
  const handleAdd = () => {
    navigate('/SecureLife.com/admin/types/new');
  };
      ...
      <Container fluid className="py-5" style={{ backgroundColor: 'rgba(230, 242, 255, 0.5)' }}>
        <Row className="m-5">
          ...
          <Col xs={6} md={6} style={{ textAlign: 'right' }}>
            <NewButton text={"Add New Insurance Type"} handleButton={handleAdd} />
          </Col>
        </Row>
        <Row className="m-5">
        ...
        <div>
          <Row className="m-5">
            <TableData data={data} fetchData={fetchData} status={"Type"} activateRow={activeType} deactivateRow={deleteType} 
              getInsideData={getSchemesByType}/>
          </Row>
          <Row className="m-5">
            <Col xs={6} md={6}>
