...
import { getAllAdmins, activateAdmin, deactivateAdmin, registerAdmin } from '../../../services/AdminService';
...
const ViewAdmin = () => {
  ...
  const activeAdmin = async (id) => {
    try {
      ...
      showToast('Error in activating admin','error');
    }
  };
  ...
  const deleteAdmin = async (id) => {
    try {
      ...
      showToast('Error in deactivating Admin','error');
    }
  };
  ...
  const handleSaveAdmin = async (formData) => {
    ...
    try {
      ...
      setLoading(false);
      let message = error?.response?.data?.specificMessage || 'Error registering admin';
  ...
  return (
    <Container fluid className="bg-light text-dark d-flex flex-column min-vh-100 px-0">
      <Header/>
      <AddAdmin show={showModal} handleClose={handleCloseModal} handleSave={handleSaveAdmin} />
        <Container fluid className="py-5" style={{backgroundColor:'rgba(230, 242, 255, 0.5)'}}>
          <Row className="m-5">
            ...
            <NewButton text={"Add New Admin"} handleButton={handleShowModal}/>
            </Col>
          </Row>
          <Row className="m-5">
            ...
            <div>
              <Row className="m-5">
                <TableData data={data} fetchData={fetchData} status={"Admin"} activateRow={activeAdmin} deactivateRow={deleteAdmin} />
              </Row>
              <Row className="m-5">
                <Col md={6}>
